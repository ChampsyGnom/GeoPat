...
namespace Emash.GeoPatNet.Infrastructure.Validations
{
    ...
    public class Validator
    {
        public static Boolean ValidateEntity<M>(M model, Dictionary<String, String> valueStrings, EntityTableInfo tableInfo, out String message)
        {
            ...
            else
            {
                ...
                foreach (String ukName in ukColumnsByUkNames.Keys)
                {
                    ...
                    {
                        EntityColumnInfo topColumn = dataService.GetTopColumnInfo(tableInfo.EntityType, ukColumnInfo.PropertyName);
                        List<EntityColumnInfo> parentUkColumnInfos =  dataService.FindParentForeignColumnInfos(topColumn);
                        if (parentUkColumnInfos.Count == 0)
                        {parentUkColumnInfos.Add(ukColumnInfo);}
                      
                        foreach (EntityColumnInfo parentUkColumnInfo in parentUkColumnInfos)
                        {
                            String path = parentUkColumnInfo.PropertyName;
                            if (!String.IsNullOrEmpty(dataService.GetPath(parentUkColumnInfo.TableInfo, ukColumnInfo.TableInfo)))
                            { path = dataService.GetPath(parentUkColumnInfo.TableInfo, ukColumnInfo.TableInfo) + "." + parentUkColumnInfo.PropertyName;}
                            
                          
                            if (!valueStrings.ContainsKey(path))
                            {
                                errors.Add(parentUkColumnInfo.DisplayName + " valeur obligatoire ");
                            }
                            else
                            {
                                ...
                                if (path.IndexOf(".") == -1)
                                {
                                    path = parentUkColumnInfo.PropertyName;
                                    expression = Expression.Property(expression,path);
                                }
                                else
                                {

                                    String[] pathItems = path.Split(".".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                                    foreach (String pathItem in pathItems)
                                    {expression = Expression.Property(expression, pathItem);}
                                }
                                
                                if (parentUkColumnInfo.ControlType == ControlType.Text)
                                {
                                  
                                    expression = Expression.Equal(expression, Expression.Constant(valueStrings[path]));
                                    expressions.Add(expression);
                                }
                                if (parentUkColumnInfo.ControlType == ControlType.Integer)
                                {
                                   
                                    Int64 valueInt64 = 0;
                                    ValidateInt64(valueStrings[path], out parseMessage, out valueInt64);
                                    expression = Expression.Equal(expression, Expression.Constant(valueInt64));
                                    expressions.Add(expression);
                                }
                                if (parentUkColumnInfo.ControlType == ControlType.Decimal)
                                {
                                   
                                    Double valueDouble = 0;
                                    ValidateDouble(valueStrings[path], out parseMessage, out valueDouble);
                                    expression = Expression.Equal(expression, Expression.Constant(valueDouble));
                                    expressions.Add(expression);
                                }
                                if (parentUkColumnInfo.ControlType == ControlType.Date)
                                {
                                  
                                    DateTime valueDateTime = DateTime.Now;
                                    ValidateDateTime(valueStrings[path], out parseMessage, out valueDateTime);
                                    expression = Expression.Equal(expression, Expression.Constant(valueDateTime));
                                    expressions.Add(expression);
                                }

                                if (parentUkColumnInfo.ControlType == ControlType.Check)
                                {
                                   
                                    Boolean valueBoolean = false;
                                    ValidateBoolean(valueStrings[path], out parseMessage, out valueBoolean);
                                    expression = Expression.Equal(expression, Expression.Constant(valueBoolean));
                                    expressions.Add(expression);
                                }
                                if (parentUkColumnInfo.ControlType == ControlType.Pr)
                                {
                                    EntityColumnInfo chausseeNavigationProperty = (from c in tableInfo.ColumnInfos where c.ColumnName.Equals(parentUkColumnInfo.PrChausseeColumnName) && c.ControlType == ControlType.Combo select c).FirstOrDefault();
                                    EntityTableInfo parentTableInfo = dataService.GetEntityTableInfo(chausseeNavigationProperty.PropertyType);
                                    ...
                                    IReperageService reperageService = ServiceLocator.Current.GetInstance<IReperageService>();
                                    Int64 abs = reperageService.PrToAbs(chausseeId, valueStrings[path]).Value;
                                    Expression expressionPr = Expression.Property(expressionBase, path);
                                    expressionPr = Expression.Equal(expressionPr, Expression.Constant(abs));
                                    expressions.Add(expressionPr);

                                }
                                if (parentUkColumnInfo.ControlType == ControlType.Combo)
                                {
                                    EntityTableInfo comboParentTableInfo = dataService.GetEntityTableInfo(parentUkColumnInfo.PropertyType);
                                    DbSet comboSet = dataService.GetDbSet(comboParentTableInfo.EntityType);
                                    IQueryable comboQueryable = comboSet.AsQueryable();
                                    List<EntityColumnInfo> comboParentNavProps = dataService.FindParentForeignColumnInfos(parentUkColumnInfo);
                                    ...
                                    foreach (String comboParentNavPropsPath in comboParentNavPropsPaths)
                                    {
                                        ...
                                        Object value = valueStrings[valuePath];
                                        expression = Expression.Equal(propertyMember, Expression.Constant(value));
                                    ...
                                    Expression expCombo = Expression.Property(expressionBase, path + "Id");
                                ...
                                Console.WriteLine("\t" + parentUkColumnInfo.ColumnName);
                            }
                        }
        ...
        public static Boolean ValidateEntityColumn(String valueString, EntityColumnInfo columnInfo, out String message, out Object value)
        {
            ...
            if (!ValidateDataType(valueString, columnInfo.PropertyType, out message, out value) && columnInfo.ControlType != ControlType.Pr)
            { return false; }
            else
            {
        ...
        public static Boolean ValidateDataRule(String valueObject, EntityColumnInfo dbProp, out String message, out Object value)
        {
            ...
            {
                if (dbProp.ControlType == ControlType.Pr)
                {
                    Int32 dplt = 10;
                    Int32 num = 0;
                    if (valueString.IndexOf("+") != -1)
